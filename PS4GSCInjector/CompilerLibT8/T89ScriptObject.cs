...
namespace T89CompilerLib
{
    ...
    public class T89ScriptMetadata
    {
        ...
        private const string T89EmbeddedMetaPath = "PS4GSCInjector.CompilerLibT8.vm_codes.db2";
            ...
            get
            {
                if(__OperationData == null)
                {
                    try
                    {
                        ...
                        byte[] tbuff;
                        using (var metadataStream = typeof(T89ScriptMetadata).Assembly.GetManifestResourceStream(T89EmbeddedMetaPath))
                        {
                            if (metadataStream == null)
                                throw new FileNotFoundException(T89PCMetaPath + " is not embedded in the application.");

                            using (var buffer = new MemoryStream())
                            {
                                metadataStream.CopyTo(buffer);
                                tbuff = buffer.ToArray();
                            }
                        }
