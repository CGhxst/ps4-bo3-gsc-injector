...
namespace PS4GSCInjector.Tests
{
    ...
    public class ConditionalBlocksTests
    {
        ...
        [TestMethod]
        public void ParseSource_UnderscoreToken_StripsWholeDirectiveIdentifier()
        {
            var cb = new ConditionalBlocks();
            cb.LoadConditionalTokens(new List<string> { "_GSC", "__PS4" });

            string input = "#ifdef _GSC\nserver();\n#endif\n#ifdef __PS4\nconsole();\n#endif";
            string output = cb.ParseSource(input);

            Assert.IsFalse(output.Contains("_GSC"));
            Assert.IsFalse(output.Contains("__PS4"));
            Assert.IsTrue(output.Contains("server();"));
            Assert.IsTrue(output.Contains("console();"));
        }
