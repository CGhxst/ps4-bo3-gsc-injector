...
namespace T7CompilerLib
{
    ...
    {
        private const string T7PCMetaPath = "ps4bo3ops.db";
        ...
        private static T7MetaV2 PCMeta
        {
            get
            {
                if (_pc_meta_ == null)
                {
                    string metaPath = T7PCMetaPath;
                    try
                    {
                        string asmDir = Path.GetDirectoryName(typeof(T7ScriptObject).Assembly.Location);
                        metaPath = Path.Combine(asmDir, T7PCMetaPath);

                        if (!File.Exists(metaPath))
                        {
                            string baseDir = AppDomain.CurrentDomain.BaseDirectory;
                            metaPath = Path.Combine(baseDir, T7PCMetaPath);
                        }

                        if (!File.Exists(metaPath) && File.Exists(T7PCMetaPath))
                        {
                            metaPath = T7PCMetaPath;
                        }

                        Deserialize(metaPath, out _pc_meta_);
                    }
                    catch (Exception e)
                    {
                        throw new InvalidOperationException(
                            $"Failed to load {T7PCMetaPath} (path: '{metaPath}'): {e.Message}", e);
                    }
                }
        ...
        public static void Serialize(string Filepath, T7MetaV2 metaRef)
        {
        ...
        /// <summary>
        private class MetaSerializationBinder : System.Runtime.Serialization.SerializationBinder
        {
            public override Type BindToType(string assemblyName, string typeName)
            {
                if (typeName.EndsWith("T7MetaV2"))
                    return typeof(T7MetaV2);
                return Type.GetType($"{typeName}, {typeof(T7MetaV2).Assembly.FullName}");
            }
        }
        ...
        public static void Deserialize(string Filepath, out T7MetaV2 metastruct)
        {
            ...
            {
                BinaryFormatter formatter = new BinaryFormatter();
                formatter.Binder = new MetaSerializationBinder();
